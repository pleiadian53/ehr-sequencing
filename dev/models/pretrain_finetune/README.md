...
### 🔄 [06_benchmarking_updates.md](./06_benchmarking_updates.md)

**Updates to benchmarking implementation**

**Topics covered:**
- Changes to benchmarking scripts (refactoring)
- New benchmark design: freeze vs fine-tune
- New benchmark: transfer learning validation
- Shared infrastructure benefits
- Why the redesign was needed
- Updated workflows and best practices

**Key updates:**
- `benchmark_embedding_finetuning.py` - Tests freeze vs fine-tune (3-way)
- `benchmark_transfer_learning.py` - Tests domain transfer (4-way)
- Shared `ehrsequencing.benchmarks.training` module
- Correct interpretation of freeze vs fine-tune
- Transfer learning as quality measure

**Read this if:**
- Using the benchmarking scripts
- Ran old `benchmark_pretrained_embeddings.py`
- Want to understand benchmarking changes
- Need migration guidance
- Writing new benchmark scripts

---

### 🔬 [07_lora_deep_dive.md](./07_lora_deep_dive.md)

**Comprehensive guide to LoRA (Low-Rank Adaptation)**

**Topics covered:**
- What is LoRA and how does it work?
- Does LoRA always freeze W? (Yes!)
- Where can LoRA apply? (Any linear layer)
- LoRA and layer normalization (order doesn't matter)
- Using LoRA with foundation models (PyTorch, HuggingFace)
- Implementation details and mathematics
- Practical examples and advanced topics

**Key concepts:**
- LoRA freezes original weights W and learns ΔW = BA
- 98% parameter reduction with minimal performance loss
- Applies to Q, K, V, output projections, FFN layers
- Transparent to layer normalization
- Works with any PyTorch model
- Enables multi-task learning via adapter swapping

**Read this if:**
- Want to understand LoRA internals
- Confused about when W gets frozen
- Need to apply LoRA to custom models
- Working with HuggingFace models
- Want to optimize parameter efficiency
...
- **Run benchmarks** → Read `06_benchmarking_updates.md` 🔄
- **Understand LoRA deeply** → Read `07_lora_deep_dive.md` 🔬
- **Modify the architecture** → Read `01_behrt_model_design.md` (sections 1-2)
- **Apply LoRA** → Read `07_lora_deep_dive.md` (comprehensive guide)
- **Debug training issues** → Read `02_training_guide.md` (section 7)
- **Tune hyperparameters** → Read `02_training_guide.md` (section 5)
- **Compare initialization strategies** → Read `06_benchmarking_updates.md` (section 2)
...
4. Check `04_clarifications_and_corrections.md` if confused ⚠️
...
4. Run benchmarks from `06_benchmarking_updates.md`

**Advanced (modifying codebase):**
1. Reference implementation details in `01_behrt_model_design.md`
2. Understand design decisions from `05_embedding_summation_and_quality_analysis.md`
3. Deep dive into LoRA from `07_lora_deep_dive.md`
4. Use benchmarking infrastructure from `06_benchmarking_updates.md`
5. Consult best practices from all documents
...
├── benchmark_embedding_finetuning.py      # Compare freeze vs fine-tune
└── benchmark_transfer_learning.py         # Test domain transfer
```
...
- "How to use new benchmarks?" → `06_benchmarking_updates.md` (section 3)
...
- "Are old results still valid?" → `06_benchmarking_updates.md` (migration guide)

### LoRA (Low-Rank Adaptation)

- "What is LoRA?" → `07_lora_deep_dive.md` (section 1) 🔬
- "Does LoRA always freeze W?" → `07_lora_deep_dive.md` (section 2) ✅ **Yes!**
- "Where can LoRA apply?" → `07_lora_deep_dive.md` (section 3) - Any linear layer
- "LoRA and layer normalization?" → `07_lora_deep_dive.md` (section 4) - Order doesn't matter
- "How to use with HuggingFace?" → `07_lora_deep_dive.md` (section 5)
- "LoRA parameter reduction?" → `07_lora_deep_dive.md` (section 6) - 98% reduction!
- "How to choose rank?" → `07_lora_deep_dive.md` (Advanced Topics)
...
| 1.3 | 2026-02-03 | Updated for benchmarking refactoring and new scripts |
| 1.4 | 2026-02-03 | Added comprehensive LoRA deep dive documentation |
