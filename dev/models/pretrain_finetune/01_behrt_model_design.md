...
#### ⚠️ Best Practice: Freeze vs Fine-tune

**Recommendation: Fine-tune embeddings by default (`freeze=False`)**

Based on benchmarking results (see `examples/pretrain_finetune/benchmark_embedding_finetuning.py`):

**Fine-tuning (recommended):**
```python
initialize_embedding_layer(..., freeze=False)  # ✅ Default choice
```
- ✅ Better performance (5-15% improvement over scratch)
- ✅ Faster convergence (20-50% fewer epochs)
- ✅ Full model capacity
- ✅ Embeddings adapt to task and model

**Freezing (rare cases only):**
```python
initialize_embedding_layer(..., freeze=True)   # ⚠️ Only for specific use cases
```
- ❌ Reduced performance (40% fewer trainable params)
- ❌ Embeddings can't adapt to new transformer
- ✅ Faster training (fewer params to update)
- ✅ Prevents catastrophic forgetting (if needed)

**When to freeze embeddings:**
- You want to prevent catastrophic forgetting
- Extremely limited compute resources
- Embeddings are proven optimal for your exact task

**When to fine-tune embeddings (default):**
- Standard training scenario
- Want best performance
- Have sufficient compute
- Transfer learning across domains

**See also:** `06_benchmarking_updates.md` for detailed comparison results
