"""
Optimized training script for discrete-time survival LSTM on RunPods GPU.

This script is specifically configured for cloud GPU training with:
- Larger datasets (1000+ patients)
- Optimized hyperparameters for A40/RTX 4090 GPUs
- Early stopping to prevent overfitting
- Learning rate scheduling
- Better model architecture for available data
...
Key differences from train_survival_lstm.py:
- No max_patients limit (uses all available data)
- Larger batch size (64 vs 32) for GPU efficiency
- Early stopping to prevent overfitting
- Learning rate scheduler
- Gradient clipping
- Model size adjusted based on dataset size
"""
